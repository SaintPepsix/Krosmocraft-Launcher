...
public class MinecraftLibrary {
	...
	public String getArtifactBaseDir() {
		...
		String[] parts = this.name.split(":", 5);
		StringBuilder baseDirBuilder = new StringBuilder();
		for (int i = 0; i < parts.length - 1; i++) {
			baseDirBuilder.append(parts[i].replaceAll("\\.", "/")).append("/");
		}
		baseDirBuilder.append(parts[parts.length - 1]);
		return baseDirBuilder.toString();
	}
	...
	public String getArtifactFilename(String classifier) {
		...
		String result;
		if (classifier == null) {
			result = String.format("%s-%s.jar", parts[parts.length - 2], parts[parts.length - 1]);
		} else {
			result = String.format("%s-%s%s.jar", parts[parts.length - 2], parts[parts.length - 1], "-" + classifier);
		}
	...
	public String getArtifactCustom(String name) {
		...
		String libName = split[split.length - 2];
		String libVersion = split[split.length - 1];
	...
	public String getPlainName() {
		String[] split = this.name.split(":", 5);
