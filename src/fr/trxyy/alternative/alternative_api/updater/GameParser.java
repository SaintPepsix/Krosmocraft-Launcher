			...
			for (int i = 0; i < nodeLst.getLength(); i++) {
				...
				if (node.getNodeType() == 1) {
					...
					if (!localFile.isDirectory()) {
								...
								if (localFile.isFile() && localFile.length() == size) {
									...
									if (!localMd5.equals(etag)) {
										if (!(engine.getGameLinks().getCustomFilesUrl() + key).endsWith("/")) {
												engine.getGameUpdater().files.put(key, new LauncherFile(size, engine.getGameLinks().getCustomFilesUrl() + key, localFile.getAbsolutePath(), etag));
												engine.getGameUpdater().filesToDownload++;
												engine.getGameUpdater().sizeToDownload += size;
										}
									}
								} else {
									if (!(engine.getGameLinks().getCustomFilesUrl() + key).endsWith("/")) {
						...
						localFile.mkdirs();
						engine.getGameUpdater().sizeToDownload += size;
						engine.getGameUpdater().sizeToBuffer += size;
					}
				}
			}
