...
public class Settings {
		    ...
		    public void handle(MouseEvent t) {
		    	...
		    	try {
					...
					try {
						...
						LauncherPanel.maxiButton.fire();//TODO fix hack
					} catch (Exception e) {
						e.printStackTrace();
					}
				} catch (Exception e) {
	...
	private LauncherImage borderlessToggle;
	private void minecraftTab() {
		...
		this.fullScreenLabel.setText("Mode plein écran sans bordures");
		...
		this.fullScreenLabel.setBounds(getResponsive(10) * 32 + getResponsive(5), getResponsive(10) * 28, getResponsive(10) * 25, getResponsive(10) * 4);
		...
		if (isSmall) {
			...
			this.fullScreenToggle.setLayoutX(getResponsive(10) * 31 + getResponsive(5));
		}
		this.fullScreenToggle.setLayoutY(getResponsive(10) * 32 + getResponsive(8));
		...
		/*this.lineMinecraft2 = new LauncherImage(root, this.loc.loadImage(theGameEngine, "grayline.png"));
		this.lineMinecraft2.setBounds(313, getResponsive(10) * 39, getResponsive(10) * 25, 1);
		this.lineMinecraft2.setOpacity(0);
    	c = new CAnimation(lineMinecraft2.opacityProperty(), 0.5, 500);
    	c.run();
    	
		this.borderlessLabel = new LauncherLabel(root);
		this.borderlessLabel.setText("Démarrer la fenêtre Minecraft en mode sans bordures");
		this.borderlessLabel.setAlignment(Pos.CENTER);
		...
		this.borderlessLabel.setBounds(getResponsive(10) * 32, getResponsive(10) * 41, getResponsive(10) * 38, getResponsive(10) * 4);
    	...
    	this.low.setOnMouseClicked(new EventHandler<MouseEvent>() {
		    ...
		    public void handle(MouseEvent t) {
		    	...
		    	if (!config.getValue(EnumConfig.QUALITY).equals("1")){
		    		...
		    		if (ram < 2) {
	    	    		config.updateValue(EnumConfig.RAM.getOption(), "2.0");
		    		}
		    	}
		    }
		});
	...
	private void javaTab() {
		...
		this.recommended2.setText("2 Go de RAM et à l'idéal 4 Go et plus");
