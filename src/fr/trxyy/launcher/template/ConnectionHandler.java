...
public class ConnectionHandler extends Service<ConnectionStatus> {
	...
	private boolean handshake(String username) {
		try {
			...
			URL obj = new URL("http://" + Infos.serverIp + ":4000/handshake");
	...
	private ConnectionStatus sendLoginPost(String username, String password) {
		try {
			URL obj = new URL("http://" + Infos.serverIp + ":4000/login");
			HttpURLConnection conn = (HttpURLConnection) obj.openConnection();
...
//	    httpget = new HttpGet("https://vote.top-serveurs.net/minecraft/vote/krosmocraft");
...
//	    result = sb.toString();
//	    result = result.replaceAll("<[^>]*>", "");
//	    if (!result.contains("VeuillezpatienterVousdevezpatienter")) {
//			Infos.vote = true;
//		}
//	    sc.close();
	}
	...
	private ConnectionStatus sendPseudoRequest(String username, String password, String pseudo) {
		try {
			URL obj = new URL("http://" + Infos.serverIp + ":4000/pseudo");
			HttpURLConnection conn = (HttpURLConnection) obj.openConnection();
